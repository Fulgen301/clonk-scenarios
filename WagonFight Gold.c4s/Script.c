...
func Script3()
{
  if (!Random(3))
    {
	if (iWagonSpeed < 65)
      SetWagonSpeed(1,true);
    if (iWagonSpeed >= 65 && iWagonSpeed < 100)
      SetWagonSpeed(2,true);
    else if (iWagonSpeed >= 100)
      SetWagonSpeed(3,true);
	}
  NewWagon();
  for (var i = 0, j = iWagonSpeed/100; i < j; i++)
    NewWagon();
  goto(1);
}
...
global func SetWagonSpeed(int float, bool add)
  {
  if (!add)iWagonSpeed = float;
  else iWagonSpeed += float;
  for (var wagon in FindObjects(Find_ID(WAGN)))
    wagon->UpdateSpeed();
  return iWagonSpeed;
  }