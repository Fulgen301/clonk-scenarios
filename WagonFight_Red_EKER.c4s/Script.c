...
public func Initialize() {
	SetNextMission("WagonFight_Red_EKER.c4s", "$restart$", "$restartdesc$");
	for (var i; i < 100; i++)
	{
	...
	SetScoreboardData(SBRD_Caption, 1, Format("{{SF5B}}"), 0);
	SetScoreboardData(SBRD_Caption, 2, Format("{{NH5B}}"), 0);
	SetScoreboardData(SBRD_Caption, 3, Format("{{AR5B}}"), 0);
...
public func InitializePlayer(iPlr)
{
	...
	RemoveObject(wipf);
	CreateObject(WAGN, GetX(GetCrew(iPlr)), GetY(GetCrew(iPlr)) + 34, -1)->SetMode(WMODE_RED);
	CreateObject(WAGN, GetX(GetCrew(iPlr)) + 40, GetY(GetCrew(iPlr)) + 34, -1)->SetMode(WMODE_RED);
	CreateObject(WAGN, GetX(GetCrew(iPlr)) - 40, GetY(GetCrew(iPlr)) + 34, -1)->SetMode(WMODE_RED);
	InitScoreboard(iPlr);
...
public func RelaunchPlayer(iPlr, iClonk, int iRelaunchs, iKiller, iKills)
{
	...
	var clnk = CreateObject(SF5B, GetX(wipf), GetY(wipf) + 3, iPlr);
	MakeCrewMember(clnk, iPlr);
	RemoveObject(wipf);
	LocalN("Relaunchs", clnk) = iRelaunchs += 1;
	LocalN("Kills", clnk) = iKills;
	if (iKiller == NO_OWNER || iKiller == iPlr)
		Log("$Relaunch$", GetTaggedPlayerName(iPlr), 20 - LocalN("Relaunchs", clnk));
	if (iKiller != NO_OWNER && iKiller != iPlr)
		Log("$RelaunchWithKiller$", GetTaggedPlayerName(iPlr), 20 - LocalN("Relaunchs", clnk), GetTaggedPlayerName(iKiller));
	DoEnergy(100, clnk);
	SelectCrew(iPlr, clnk);
	SetPosition(GetX(clnk), GetY(clnk) + 10, clnk);
	CreateObject(WAGN, GetX(clnk), GetY(clnk) + 34, -1)->SetMode(WMODE_RED);
	CreateObject(WAGN, GetX(clnk) + 40, GetY(clnk) + 34, -1)->SetMode(WMODE_RED);
	CreateObject(WAGN, GetX(clnk) - 40, GetY(clnk) + 34, -1)->SetMode(WMODE_RED);
...
global func NewWagon()
{
	...
	/*for(var obj in FindObjects(Find_ID(SF5B)))
{
...
//Spawnschutz 
global func FxSpawnTimer(pTarget, iEffectNumber, int iEffectTime)
{
...
global func InitScoreboard(int iPlr, bool Erase, int iKills)
{
	...
	SetScoreboardData(iPlr, 2, Format("%d", 20 - LocalN("Relaunchs", GetCrew(iPlr))), LocalN("Relaunchs", GetCrew(iPlr)) + 1);
