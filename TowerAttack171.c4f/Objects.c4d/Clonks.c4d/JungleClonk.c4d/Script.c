...
public func ControlThrow() 
{
  ...
  // Anfassen: keine �berladene Steuerung
  if(GetAction()S="Push")
    return(0);
...
public func ControlLeft()  { if (ControlVine("ControlLeft")) return(1); return(Ctrl2ID("ControlLeft"));  }
public func ControlRight() { if (ControlVine("ControlRight")) return(1); return(Ctrl2ID("ControlRight")); }
public func ControlUp()    { if (ControlVine("ControlUp")) return(1); return(Ctrl2ID("ControlUp"));    }
public func ControlUpdate(object pByObject, int comdir, bool dig, bool throw) { if(ControlVine("ControlUpdate", comdir, dig, throw)) return(1); return(Ctrl2ID("ControlUpdate", comdir, dig, throw)); }
...
{ 
  ...
  return(Ctrl2ID("ControlDown"));  
}

public func ControlDigDouble()          // Der Urwaldclonk kann B�ume instant-zerlegen
{
  // Bedienung im Geb�ude
...
{
  // Der Urwaldclonk kann zus�tzlich noch 18 Pfeile tragen (also 3 Packs)
...
protected func CheckChop() {            // EndCall von Chop2 (B�ume zerlegen)
  var pA = GetActionTarget();
  // Aktionsziel auf mysteri�se Art und Weise verschwunden
  if(pA)
...
private func IsWeaponReady()            // ist bereit f�r das Benutzen von Waffen
{
...
private func CheckArmed()           // Bewaffnung (mit Speer) pr�fen
{
...
protected func Collection(pObj)
  ...
  // Ist das aufgenommene Objekt �berhaupt packbar?
  ...
  if (ContentsCount(GetID(pObj)) >= iPackCount)
    // Warum nur nicht ComposeContents? - K�nnte auch gehen, ausprobieren, testen.
    PackContents(iPackCount, GetID(pObj), idPackTo);
  return(1);
  }

/* �berpr�ft ob Par(0) ein Pfeil-ID ist  */
private func CheckArrow(idDef) { return(idDef->~IsArrow()); }

/* �berpr�ft ob Par(0) ein Pfeilpaket-ID ist */
...
protected func StartHandcraft(id def) {
  ...
  // Hammer wird ben�tigt
  if (!FindContents(MLLT)) NeedMallet ();
}
...
private func Handcraft() {
  ...
  // Zielobjekt in eigenen Beh�lter bef�rdern
...
func GetProducerOf (def) {
  ...
  // Allerdings keinen Hammer, denn dazu br�uchte er einen Hammer
...
/* Baumsuchen (S�gewerkscript) */

private func FindWorkTree(object clonk)
{
  // Arbeiter f�llt bereits einen Baum
  if (GetAction(clonk) eq "Chop") 
    return(GetActionTarget(0, clonk));
  // Arbeiter hat bereits einen Baum angefa�t
...
private func FindTreeToChop()
{
  ...
  while (obj = FindObject(0,0,0,-1,-1,OCF_Chop(),0,0,0,obj))
    ...
    // Brennt nicht
    if (!OnFire(obj))
    // Baum wird nicht bereits von einem Clonk gef�llt
...
private func FindTreeToGrab()
{
  ...
  while (obj = FindObject(0,0,0,-1,-1,OCF_Grab(),0,0,0,obj))
    ...
    // Steckt nicht fest
    if (!Stuck(obj))
    // Baum wird nicht bereits von einem Clonk angefa�t
