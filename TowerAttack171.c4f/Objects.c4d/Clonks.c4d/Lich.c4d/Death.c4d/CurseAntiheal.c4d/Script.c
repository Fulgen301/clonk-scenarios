...
protected func FxCurseAntihealDamage(pClonk, iEffectNumber, iDmgEngy, iCause)
{
  ...
  if(iDmgEngy>0)
  {
    ...
    CreateParticle("MSpark",GetX(pClonk)+RandomX(-8,8),GetY(pClonk)+RandomX(-10,10),0,10,RandomX(18,38),RGB(20,20,210));
    CreateParticle("PSpark",GetX(pClonk),GetY(pClonk),0,0,180,RGB(0,8,16),pClonk);
    return(-Random(iDmgEngy+1));
  }
...
public func GetSpellStaff(object pMage) { if(GetID()==CAHE) return(SMOD); }
public func GetSpellStaffCombo(pMage) { return ("13"); } // (1: Backward; 2: Down; 3: Forward; 4: Throw; 5: Jump; 6: Dig)
