...
func Initialize() {
   ...
   SetGamma (RGB(5,0,2), RGB(168,128,148), RGB(255,222,245)); 
   Log("Ich mag Züge.");
...
func InitializePlayer(iPlr)
{
   ...
   if(!FindObject(WAGN)){ CreateObject(WAGN,LandscapeWidth()/2,Position(),-1); Log("Bugselfreport: No Waggons Player Join"); }
...
global func CreateTheTrain(){
    var wag;
    wag=CreateObject(STMG,100,100,-1);

}
func Menu()
{
    menu=1;
    CreateMenu(QSMI,GetCrew(0),0,0,"Spielmodus");
    AddMenuItem("Normal","Normal",WAGN,GetCrew(0),0,0,"Waggons und Züge");
    AddMenuItem("Zugfight","Doof",STMG,GetCrew(0),0,0,"Nur Züge");
}

func Normal()
{
Message("Waggonfight Insane Train Mod aktiviert.");
    return();
}

func Doof()
{
 for(var i in FindObjects(Find_ID(WAGN),Find_Func("IsNotTrainpart")))
     {
       if(!GetEffect("Fade",i)){
        if(!GetEffect("FadeIn",i)){
        // i->Fade();
        }
       }
      }
imod = 1; 
Message("Zugfight Mod aktiviert.");
...
func RelaunchPlayer(iPlr,iClonk,iRelaunchs, iKiller, iKills)
{
    ...
    if(!FindObject(WAGN)){ CreateObject(WAGN,LandscapeWidth()/2,Position(),-1); Log("Bugselfreport: No Waggons Player Relaunch"); }
    var wipf=PlaceAnimal(WIPF);
...
global func GetScenDir(){
if(!imod){ return(dir); }
if(imod){ return(RandomX(0,1)); }
}
func Script3()
{
  if(imod)  goto(1);
