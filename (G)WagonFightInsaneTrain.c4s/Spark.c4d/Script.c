...
protected func CastObject()
{
 ...
 if(!Random(30)){ Randi = 5; }
 if(Randi == 1) CreateObject(ID, 0,0, -1)->SetMagic(CDIR);
 if(Randi == 2) CreateObject(ID, 0,0, -1)->SetMagic(SWGN);
 if(Randi == 3) CreateObject(ID, 0,0, -1)->SetMagic(STRN);
 if(Randi == 4) CreateObject(ID, 0,0, -1)->SetMagic(MQKE);
 if(Randi == 5) CreateObject(ID, 0,0, -1)->SetMagic(MMTR);              
 Sound("Boing"); 
 return(1);
  }
